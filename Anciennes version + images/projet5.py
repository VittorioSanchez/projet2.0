...
class StatusDisplay(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.text = "SHIELD HITS: %d   MISSILE HITS: %d  WAVE NUMBER: %d MUNITIONS: %d SCORE %d" % (self.lives, self.score, self.wave, self.munition, points)
    ...
    def update(self,lives,score,wave):
        ...
        if milliseconds > 1000:
            ...
            self.seconds += 1
            milliseconds -= 1000
        points = (self.score)*10 + self.seconds            
        self.text = "SHIELD HITS: %d   MISSILE HITS: %d  WAVE NUMBER: %d MUNITIONS: %d SCORE: %d" % (self.lives, self.score, self.wave,self.munition, points)
        self.image = self.font.render(self.text, 1, (0, 255, 0))
        self.rect = self.image.get_rect()
                    ...
                    if event.key == K_RETURN:
                        ...
                        if menu.get_position() == 2:#here is the Menu class function
                           #lorem ipsum ici les règles
                            ...
                            menu.draw()
                            text = """Les déplacements d'effectuent avec les flèches du clavier. 
Vous disposez de trois vies 
Appuyez sur la touche W de votre clavier pour tirer

SCORE : 
Vous marquez un point par seconde en vie et 10 points par ennemi touché 
Faites le meilleur score possible ;) 

Attentions vos munitions sont limitées (50 au départ) !
Des caisses sont là pour vous aider il en existe de deux types et 
apparaissant aléatoirement : l'une vous donnera 25 munitions, 
l'autre vous offrira des réparations (1 vie récupérée) 
